...
def generate_recommendation_copy(
    ...
    prompt = (
        ...
        "For a long laptop/focus session, the summary must aggregate only supplied playlist facts: low-stimulation or calm anchors, piano or ambient tracks when supplied, and a light-rhythm track only when at least one supplied track has light_rhythm_fit. Never describe the entire playlist as having a steady rhythm. Prefer a two-part summary such as calm instrumentals plus '가벼운 리듬감이 느껴지는 곡도 함께 담았어요'; do not write the awkward phrase '지나치지 않은 리듬감'. Do not write '집중할 수 있도록', '몰입을 유지', '집중력을 높여', '흐름을 유지시켜', or '산만함을 막아'.\n"
